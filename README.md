...
Worktree first-run bootstrap (env-file copy + `just bootstrap`) is **not** a lefthook — each launch path owns its own logic. Zed's `create_worktree` task ([`.zed/tasks.json`](.zed/tasks.json)) inlines the cp then calls `just bootstrap`. Claude Code's `SessionStart` hook ([`.claude/settings.json`](.claude/settings.json)) calls `just bootstrap` against env files already copied by [`.worktreeinclude`](.worktreeinclude). Codex runs inside Zed-spawned worktrees so it inherits the bootstrap. `just bootstrap` itself wraps `bun install` with an mkdir-lock so concurrent Zed-task + Claude-SessionStart invocations on the same fresh worktree serialize instead of racing on `clonefileat`. Per [ADR 0015](docs/decisions/0015-dotenvx-secrets-management.md), per-developer `.env.local` and `.env.keys` are listed in `.worktreeinclude` and copy in alongside the worktree. Note: `claude --worktree` branches the new worktree from `origin/HEAD` (not your current local branch) — see the root [`AGENTS.md`](AGENTS.md) for the base-ref alignment dance.
