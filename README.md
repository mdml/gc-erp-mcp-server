...
Worktree first-run bootstrap (env-file copy + `bun install`) is **not** a lefthook — it runs from Zed's `create_worktree` task and Claude Code's / Codex's `SessionStart` hook via [`scripts/bootstrap.sh`](scripts/bootstrap.sh). Per [ADR 0015](docs/decisions/0015-dotenvx-secrets-management.md), per-developer `.env.local` and `.env.keys` are listed in `.worktreeinclude` and copy in alongside the worktree — no secret-sync step needed. Note: `claude --worktree` branches the new worktree from `origin/HEAD` (not your current local branch) — see the root [`AGENTS.md`](AGENTS.md) for the base-ref alignment dance.
