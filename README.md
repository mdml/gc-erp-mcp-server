...
Worktree first-run bootstrap (env-file copy + `bun install`) is **not** a lefthook — each launch path owns its own logic. Zed's `create_worktree` task ([`.zed/tasks.json`](.zed/tasks.json)) inlines the cp + bun install. Claude Code's `SessionStart` hook ([`.claude/settings.json`](.claude/settings.json)) runs `bun install` against env files already copied by [`.worktreeinclude`](.worktreeinclude). Codex runs inside Zed-spawned worktrees so it inherits the bootstrap. Per [ADR 0015](docs/decisions/0015-dotenvx-secrets-management.md), per-developer `.env.local` and `.env.keys` are listed in `.worktreeinclude` and copy in alongside the worktree. Note: `claude --worktree` branches the new worktree from `origin/HEAD` (not your current local branch) — see the root [`AGENTS.md`](AGENTS.md) for the base-ref alignment dance.
