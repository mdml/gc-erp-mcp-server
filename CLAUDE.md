...
- **`main` is PR-only, linear-history, rebase-merge only.** GitHub branch protection enforces all three: direct pushes, merge commits, and squash-merges are rejected server-side. Use `gh pr merge <n> --rebase --delete-branch`. Commitlint at commit time is what makes rebase-merge safe as the default — PR branches arrive with meaningful conventional-commit history so rebase preserves real signal on `main`. If a rare PR genuinely needs squashing (WIP-heavy branch), Max has to temporarily relax the protection rule; surface the need rather than trying to work around it.
