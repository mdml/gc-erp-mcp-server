...
- **Missing required IDs disable Save. Don't synthesize defaults.** Pre-fill-only by design — if `structuredContent` didn't supply `scopeId`, `commitmentId`, `activityId`, or `counterpartyId`, the Save button stays disabled and the missing-hint renders. No picker, no "scope not chosen yet" flow — that's deferred to whichever scope adds richer UI (likely [scope 3 — Schedule dependencies](../../docs/product/scope/3-schedule-deps.md)).
- **`CostSource` is mirrored structurally, not imported.** Importing from `@gc-erp/database` would pull zod + drizzle into the view bundle. If `CostSource` grows a variant *that the form can produce*, update `form.ts` in the same PR (both sides change together, same commit). Fields the form can't produce yet — `documentId` being the current example — are intentionally out of the mirror; wire them when the corresponding UI lands, not before.
...
- **React (yet).** Plain DOM wiring is fine for one form. When the next app lands a more complex tree (e.g. a [scope 3](../../docs/product/scope/3-schedule-deps.md) schedule dashboard), evaluate `@modelcontextprotocol/ext-apps/react` + `useApp` at that point — not now.
