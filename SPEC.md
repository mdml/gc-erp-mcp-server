...
Resolve before coding (or explicitly defer).

### Schema / modeling

- **Scope taxonomy.** CSI codes vs. custom lightweight list vs. free-form. Commercial CSI feels heavy for residential custom. Leaning: free-form `name` + optional `code`, with a curated list of common scopes as seed data. Defer the CSI decision.
- **Direct-materials escape hatch.** Requiring every Cost to have a Commitment is clean but annoying for petty cash / direct buys. Option: a convention where a single "self-commitment" per job absorbs all direct materials, auto-created. Good enough?
...
- **Billed vs. paid.** Currently nothing in the schema tracks billing/payment state. Add `PayApp` as a first-class event (pay apps are numbered, approved, paid)? Add `Payment` as another append-only event type referencing a Cost or a PayApp line?
- **Activation without pricePortion for unit-priced commitments.** Schema says `pricePortion: Money` — for unit-priced, is it the *expected* portion? Need to nail this down.
- **Retainage.** Where does it live? A field on Commitment? A field on each pay app line? Per-job default?
- **Closing an activation.** When is it "done"? Operator flips a flag? Last cost lands? This matters for variance computation.

### Patch / event-sourcing mechanics

- **Patches and concurrency.** For dogfood, single-operator is fine. But two of us on one job eventually means two people might patch at once. Git-style branching? Linear chain with optimistic concurrency? Out of v1 scope probably.
- **Patch granularity.** Should a patch span multiple jobs (e.g. a project-level CO affecting two jobs)? Current schema says no (jobId on Patch).
- **Patch authoring UX.** Does the operator consciously batch edits into a patch, or does the server auto-group edits with the same intent (e.g. within a session)?

### Storage & runtime

- **Storage backend.** Postgres vs SQLite+Litestream vs plain JSON on disk vs event log as files + materialized view. The commitment state = fold(patches) model argues for event log + materialized state. Decide before M1.
- **Where does the server run?** Local process (npx), hosted, or both? Affects auth/multi-device story eventually.

### Project ↔ job

- **Project-level commitments.** Architect, GL insurance, permitting fees often span multiple jobs at a project. v1 says commitments live on jobs. When we have >1 job per project, this will bite. Ignore until it does?
- **Contract roll-down.** Client contract is often at project level but billing happens per job. How does that surface in v1? Probably: just a `contractRef` field on Project, with pay apps still job-level.

### MCP / runtime

- **Which MCP apps ship first?** Likely cost-entry form (M3). Then job dashboard (M4), pay app generator (M5).
- **How does Claude choose an app?** Based on tool return type? Based on explicit hints? Per the apps-extension spec, worth re-reading before M3.
- **Skill docs.** One living `SPEC.md` (this file) or one `SKILL.md` per module? Initial bias: keep this spec singular until it gets unwieldy.

### Out of scope reminders (not questions, but worth restating)

- Plans + Options: schema slots only in v1.
- Home-buyer website, subs portal, CRM, ledger logic, multi-tenant: all deferred.
