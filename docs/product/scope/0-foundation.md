...
- `.zed/tasks.json` with `create_worktree` hook inlining env-file copy + `just bootstrap`. `claude --worktree` flow covered separately by `.worktreeinclude` (env-file copy) + a `SessionStart` hook (`just bootstrap`). Both launchers funnel through `just bootstrap` for the install step — the recipe wraps `bun install` in an mkdir-lock so concurrent Zed-task + Claude-SessionStart fires on a fresh worktree serialize instead of racing on `clonefileat`. Zed-as-agent-host framing captured in `docs/guides/zed.md`.
- `packages/agent-config` deleted; `bun install` hook removed; per-harness permission surfaces hand-maintained: narrow `.claude/settings.json` for Claude Code; `.codex/config.toml` + Starlark `.codex/rules/*.rules` for Codex (per Codex's `prefix_rule` grammar). Codex specifics + the trust-gate footgun captured in `docs/guides/codex.md`.
...
3. **Zed task glue + worktree bootstrap.** `.zed/tasks.json` with `create_worktree` hook inlining the env-file copy then calling `just bootstrap` (`cd $ZED_WORKTREE_ROOT && cp -n $ZED_MAIN_GIT_WORKTREE/.env.{local,keys} . && just bootstrap`). `claude --worktree` covered separately by `.worktreeinclude` (env-file copy) + a `SessionStart` hook in `.claude/settings.json` also calling `just bootstrap`. Both launchers funnel through `just bootstrap` for the install step — the recipe wraps `bun install` in an mkdir-lock so concurrent fires on a fresh Zed-with-Claude worktree serialize instead of racing on `clonefileat`. Each launch path still owns its env-file glue (no Zed-specific concerns in shared code). Earlier iterations both produced flake: a shared `scripts/bootstrap.sh` with Zed-specific env-var lookups (flaky Zed task firing, fixed by inlining per launch path); and the lock-less inlining that followed (half-linked `node_modules` when both launchers fired in parallel, fixed by the mkdir-lock in `just bootstrap` — [PR #52](https://github.com/mdml/gc-erp-mcp-server/pull/52)). `docs/guides/zed.md` written per the vendor-guide convention (Agent Panel, external agents via ACP, `create_worktree` task hook, "we don't use Zed Agent thread type" boundary).
4. **Sunset `packages/agent-config` + add Codex permission surface.** Delete the package; remove the `bun install` hook from `package.json`'s `prepare` script; narrow `.claude/settings.json` to a small hand-maintained file lifting current allow/deny verbatim. Add `.codex/config.toml` (with `approval_policy`, `sandbox_mode`, `[mcp_servers.*]`) and `.codex/rules/*.rules` (Starlark `prefix_rule` translations of the Claude allowlist). `docs/guides/codex.md` written per the vendor-guide convention, including the trust-gate footgun (repo-local `.codex/config.toml` does not load until the user accepts the trust prompt on first run).
