...
- `.zed/tasks.json` with `create_worktree` hook inlining env-file copy + `bun install`. `claude --worktree` flow covered separately by `.worktreeinclude` (env-file copy) + a `SessionStart` hook (`bun install`). Zed-as-agent-host framing captured in `docs/guides/zed.md`.
...
3. **Zed task glue + worktree bootstrap.** `.zed/tasks.json` with `create_worktree` hook inlining the bootstrap commands directly (`cd $ZED_WORKTREE_ROOT && cp -n $ZED_MAIN_GIT_WORKTREE/.env.{local,keys} . && bun install`). `claude --worktree` covered separately by `.worktreeinclude` (env-file copy) + a `SessionStart` hook in `.claude/settings.json` running `bun install`. No shared bootstrap script — each launch path owns its own glue (an earlier shared-script design produced flaky Zed task firing; inlining fixed it). `docs/guides/zed.md` written per the vendor-guide convention (Agent Panel, external agents via ACP, `create_worktree` task hook, "we don't use Zed Agent thread type" boundary).
