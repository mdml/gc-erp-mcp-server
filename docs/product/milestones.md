...
- **M3 — First MCP app (UI component).** Ship a cost-entry form as an MCP app. Prove the pattern: Claude chooses the form, pre-fills from context, user confirms. *(Complete 2026-04-20 — Desktop dogfood verified the full round-trip: `cost_entry_form` tool → pre-filled iframe view → Save-button attestation gates `record_cost` → D1 row. Unexpected find: vendor guide §6.8's `oninitialized`-deferred registration broke `resources` capability advertisement — caught, fixed in PR #38, guide corrected.)*
