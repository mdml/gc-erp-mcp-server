...
- **`install:mcp:local` shipped + dogfood doc corrected** (2026-04-18): `bun run install:mcp:local` patches `~/Library/Application Support/Claude/claude_desktop_config.json` with the `gc-erp-local` entry, pinning `command` to Homebrew's `/opt/homebrew/bin/npx` (Node ≥18 requirement of `mcp-remote`'s `wsl-utils`). Backs up the existing config, preserves other servers + preferences, supports `--remove`. Dogfood doc previously claimed Desktop supports native `type: "http"` "as of early 2026" — [verified against official MCP docs](https://modelcontextprotocol.io/docs/develop/connect-local-servers) it does not; the config file is stdio-only. `install:mcp:prod` still TBD; for prod the in-app Connectors UI is the recommended path (HTTPS only, originates from Anthropic — won't work for localhost, hence the bridge for local). Crosses off ~half of item 2 below.
