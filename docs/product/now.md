...
1. **M2 core tools — commitment + NTP arc.** The verbs the kitchen walkthrough still needs: `apply_patch` (commitment edits + change orders), `issue_ntp`, `record_cost`, `record_direct_cost`. Each lands with its Day N block in `kitchen.ts`. See [TOOLS.md §3](../../TOOLS.md). Day 3 (`create_party` + `apply_patch`) is blocked on `apply_patch`.
