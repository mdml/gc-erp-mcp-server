...
1. **Build the portability layer** ([scope 0](scope/0-foundation.md), slice A — single PR `feat/portability-layer`, sub-tasks ordered lowest-risk first):
   1. Justfile at root: `just check`, `just test`, `just bootstrap`, `just deploy`, `default → just --list`. Recipes shell out to current `bun` / `turbo`; dotenvx is the inner wrap on any recipe that touches a secret (per [scope 0](scope/0-foundation.md) §Secrets handling). Rust recipes added at P1.
   2. `AGENTS.md` symlinked to `CLAUDE.md`.
   3. Zed configured for multi-agent (Claude Code + Codex). Capture in `docs/guides/zed-multi-agent.md`.
   4. Sunset `packages/agent-config` (delete + remove `bun install` hook; narrow `.claude/settings.json` to a hand-maintained file lifting current allow/deny verbatim).
2. **Start P1 — Rust-MCP POC** ([scope 0](scope/0-foundation.md), slice B phase P1): rename `apps/mcp-server/` → `apps/mcp-server.ts-archive/`; spike `axum` + `rmcp` + Clerk JWT + `sqlx`/Postgres deployed to Fly.io; write [`docs/guides/rust-mcp.md`](../guides/rust-mcp.md).
