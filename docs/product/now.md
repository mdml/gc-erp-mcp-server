...
- **`install:mcp:local` + sync-secrets `localDevVars`** (2026-04-18): `bun run install:mcp:local` patches `~/Library/Application Support/Claude/claude_desktop_config.json` with the `gc-erp-local` entry, pinning `command` to Homebrew's `/opt/homebrew/bin/npx` AND injecting `PATH=/opt/homebrew/bin:/usr/bin:/bin` into the spawned process's env — pinning `command` alone wasn't enough because `npx` is `#!/usr/bin/env node` and re-resolves `node` from PATH at runtime, landing on Desktop's launch-services PATH (which surfaces stale nvm Node 16 first → `wsl-utils` syntax error). `secrets.config.ts` now exposes a `localDevVars` map of fixed-literal `.dev.vars` values (`MCP_BEARER_TOKEN=dev`); `sync-secrets` always overrides resolved 1Password values with these literals so local stays local. Dropped `dev-vars` from team `MCP_BEARER_TOKEN`'s targets — prod token now flows only to `.envrc.enc`. Dogfood doc updated; previously claimed Desktop supports native `type: "http"` "as of early 2026" — [verified against official MCP docs](https://modelcontextprotocol.io/docs/develop/connect-local-servers) it does not (config file is stdio-only). `install:mcp:prod` still TBD; for prod the in-app Connectors UI is the recommended path. Crosses off ~half of item 2 below.
