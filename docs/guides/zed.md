...
[
  {
    ...
    "args": [
      "-c",
      "set -e; export PATH=\"$HOME/.bun/bin:/opt/homebrew/bin:$PATH\"; cd \"$ZED_WORKTREE_ROOT\"; cp -n \"$ZED_MAIN_GIT_WORKTREE/.env.local\" .env.local 2>/dev/null || true; cp -n \"$ZED_MAIN_GIT_WORKTREE/.env.keys\" .env.keys 2>/dev/null || true; just bootstrap"
    ],
...
The `cp -n` with `|| true` makes the copy step tolerant of missing source files (e.g. when the main worktree itself doesn't have `.env.local` set up yet). `just bootstrap` wraps `bun install` (no-op when the lockfile is satisfied) with an mkdir-lock so a concurrent Claude `SessionStart` invocation on the same fresh worktree serializes instead of racing on `clonefileat` — past incident: parallel installs produced a half-linked `node_modules` with only the top-level binaries materialized.
...
**`claude --worktree`** (the Claude Code CLI worktree flow) is handled separately by [`.worktreeinclude`](../../.worktreeinclude) (env-file copy) plus a `SessionStart` hook in [`.claude/settings.json`](../../.claude/settings.json) (also `just bootstrap`). Different launch path, different glue — no shared script. Both launchers funnel through `just bootstrap` for the install step; the mkdir-lock inside the recipe is what keeps concurrent fires safe. **Manual fallback** for any worktree spawned outside an agent harness: `just bootstrap`.
