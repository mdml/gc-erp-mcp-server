...
- Two new secrets (`STYTCH_PROJECT_ID`, `STYTCH_SECRET`) join the 1Password `gc-erp` vault and need `turbo.json` `globalPassThroughEnv` declarations — Turbo 2.x strips env vars by default (per [`packages/dev-tools/CLAUDE.md`](../../packages/dev-tools/AGENTS.md)).
...
- **Verify `mcp-remote` + streamable-HTTP + Stytch OAuth works end-to-end from Claude Desktop.** Claude Desktop's `claude_desktop_config.json` is stdio-only, so the prod entry bridges through `mcp-remote` ([`docs/guides/dogfood.md §install:mcp:prod`](../guides/dogfood.md)). `mcp-remote` handles the MCP OAuth flow natively (DCR → browser consent → token cache) — but this is an untested combination for us, paralleling the SSE-vs-streamable-HTTP flag above. Before declaring prod green, smoke-test a real Desktop-via-`mcp-remote` → Stytch consent (email OTP) → `list_jobs` round-trip. Failure modes to watch: (a) `mcp-remote` doesn't yet speak streamable HTTP (only SSE), (b) the OAuth discovery step chokes on something in our `/.well-known` response shape, (c) token caching under `~/.mcp-auth/` races with Desktop's restart-per-edit lifecycle.
