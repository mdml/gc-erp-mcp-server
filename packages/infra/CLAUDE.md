...
Remote Cloudflare provisioning for gc-erp-mcp-server. Counterpart to `packages/dev-tools/`, which owns *local* dev env (gate runner, code-health, scenario runner). Infra owns *remote* state — custom domain, D1, and R2 today; Worker-side secrets will follow as the secrets provider lands. Never bundled into the runtime.
...
- **Auth from env, never cached.** `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` are read from `process.env` at call time. Per [ADR 0015](../../docs/decisions/0015-dotenvx-secrets-management.md), wrap infra commands in `bunx dotenvx run -f .env.local --` to inject them — see [docs/guides/ARCHITECTURE.md §4](../../docs/guides/ARCHITECTURE.md). No `wrangler login`, no OAuth cache.
- **Secrets will never land in memory state.** When the secrets provider is added, values stream from `bunx dotenvx get` stdout directly into `wrangler secret put` stdin via a `Blob` wrap (`Bun.spawn` rejects raw strings with a cryptic `TypeError`). No temp files, no logs, no intermediate variables.
...
- **A dispatcher/command framework.** Each top-level command is its own `bun run infra:<cmd>` entry with plain `process.argv` parsing. Matches the flat shape of `gate` / `code-health` in `packages/dev-tools/`.

## Development

```bash
bunx dotenvx run -f .env.local -- bun run infra:status                # read-only; exits 0 only if everything is OK
bunx dotenvx run -f .env.local -- bun run infra:apply                 # prints the plan; exits 0 without mutating
bunx dotenvx run -f .env.local -- bun run infra:apply --yes           # executes the plan
bunx dotenvx run -f .env.local -- bun run infra:teardown --force      # destructive; detaches custom domain, deletes D1/R2 (+ later: removes secrets)
```

Prereqs: `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` set in your encrypted `.env.local` (`bunx dotenvx set …`). The dotenvx wrapper injects them into the bun subprocess; see [docs/guides/ARCHITECTURE.md §4](../../docs/guides/ARCHITECTURE.md) for the secrets flow and [ADR 0015](../../docs/decisions/0015-dotenvx-secrets-management.md) for the rationale.
